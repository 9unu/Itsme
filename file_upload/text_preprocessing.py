...
def txt_to_csv(file, user_name):
    ...
    past_pattern = r"(\d{4}년 \d{1,2}월 \d{1,2}일) (오후|오전) (\d{1,2}:\d{2}), (.*?) :"
    match = re.search(past_pattern, lines[30].decode('utf-8').strip())
    """ 방 이름 확인 """
    if match:
        room_name = lines[0].decode('utf-8').strip().replace(" 카카오톡 대화", "")
    else:
        room_name = lines[0].decode('utf-8').strip().replace(" 님과 카카오톡 대화", "")
